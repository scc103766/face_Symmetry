...
from .schema import MEDIAPIPE_FACE_LANDMARKS, FaceKeypointDetection


class MediaPipeUnavailableError(RuntimeError):
    """Raised when the MediaPipe Tasks runtime is not available."""


class MediaPipeFaceLandmarkerDetector:
    """MediaPipe Tasks Face Landmarker adapter for static image keypoint detection."""
        ...
        *,
        ...
        try:
            ...
            raise MediaPipeUnavailableError(
                "MediaPipe Tasks runtime is not available. Install `mediapipe` in the runtime environment."
            ) from exc
    ...
    def detect_image_path(self, image_path: Path, *, image_id: str | None = None) -> FaceKeypointDetection | None:
        try:
    ...
    def detect_rgb_image(self, image: Any, *, image_id: str) -> FaceKeypointDetection | None:
    ...
    def _to_detection(self, result: Any, *, image_id: str) -> FaceKeypointDetection:
        ...
        named = {
            ...
            for name, index in MEDIAPIPE_FACE_LANDMARKS.items()
            if index < len(raw_landmarks)
        }
        return FaceKeypointDetection(
            image_id=image_id,
            landmarks=named,
    ...
    def _blendshapes(self, result: Any) -> dict[str, float]:
        ...
        if not blendshapes:
            return {}
        return {category.category_name: float(category.score) for category in blendshapes[0]}

    def _transformation_matrixes(self, result: Any) -> list[list[list[float]]]:
        matrixes = getattr(result, "facial_transformation_matrixes", None) or []
        return [matrix.astype(float).tolist() for matrix in matrixes]
